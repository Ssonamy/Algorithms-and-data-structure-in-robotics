...
void merge(vector<int>& arr, int left, int mid, int right) //Вспомогательная функция для сортироовки слиянием
{
	int i = left, j = mid + 1;
	vector<int> temp;
...
void mergeSort(vector<int>& arr, int left, int right) // Сортировка слиянием: делит массив на две части, сортирует каждую часть отдельно (рекурсивно), а потом сливает их обратно в один отсортированный массив.
{
	if (left < right) {
